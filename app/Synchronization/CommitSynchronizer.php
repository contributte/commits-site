	...
	): void
	{
		...
		foreach ($repositories as $index => $repository) {
			if ($onRepositoryStart !== null) {
				$onRepositoryStart($index, $repository);
			}
		...
		foreach ($paginator as $response) {
			...
			foreach ($commits as $commit) {
				...
				if ($this->existsCommit($repository, $commit->sha)) {
					...
					$deleted = $this->unreachableCommitsDeleter->delete($repository, $this->shaMap[$repository->getID()], $commit->sha);
					...
					$this->commitsOrderUpdater->update($repository, $this->shaMap[$repository->getID()], $newSHAs);
